...
function getIntent(message) {
    ...
    if (/\b(product|products|item|items|shop|buying|category|categories|t-shirt|tshirts|tshirts|tshirts|shirts|jeans|shorts|hoodies)\b/.test(m)) return 'products';
    ...
    switch (intent) {
        ...
        case 'products':
            ...
            categories.forEach(cat => {
                categoryHtml += `
                            <button data-category-id="${cat.id}" data-category-name="${cat.name}" class="category-btn bg-(--bg-cream) hover:bg-(--primary) hover:text-white text-(--main-text) px-3 py-1.5 rounded-full text-xs font-bold transition-all shadow-sm active:scale-95">
                                ${cat.name}
                            </button>
                        `;
            });
        ...
        case 'shipping':
            return `
                        <div class="flex flex-col gap-1">
                            ...
                            <span class="text-xs text-(--primary) font-bold">Free shipping on orders over $50!</span>
                        </div>
                    `;
        case 'order':
            ...
            userOrder.forEach(item => {
                orderHtml += `
                <div class="space-y-3 bg-(--bg-cream) p-3 rounded-2xl">
                    ...
                    <button data-order-id="${item.id}" id="downloadInvoice" class="download-invoice-btn w-full mt-2 bg-(--primary) text-white py-3 px-4 rounded-xl text-xs font-bold hover:opacity-90 transition flex items-center justify-center gap-2 shadow-xl transform active:scale-95">
        ...
        top3.forEach(p => {
            productHtml += `
                <a href="index.html#product?id=${p.id}">
                    <div class="group cursor-pointer flex items-center gap-3 bg-(--bg-cream) p-2 rounded-xl hover:border-(--primary) transition">
                        <img src="${p.mainImage}" class="w-12 h-12 object-cover bg-(--main-bg) rounded-lg p-1 shadow-sm" alt="${p.name}">
                        <div class="flex-1 min-w-0">
                            <p class="text-xs font-bold text-(--main-text) truncate">${p.name}</p>
                            <p class="text-xs text-(--primary) font-extrabold">$${p.price}</p>
                        </div>
                    </div>
                </a>
            `;
